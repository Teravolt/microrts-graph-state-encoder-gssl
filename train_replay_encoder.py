...
from nn.replay_encoder import GNNReplayEncoder
...
def create_model(node_dims: int, edge_dims: int, hidden_dims: int, _config: Namespace):
    ...
    :param config: Script config
    :returns: GNN replay encoder model
    """

    replay_encoder = GNNReplayEncoder(node_dims, edge_dims, hidden_dims)
    return replay_encoder
    ...
    for epoch in range(config.num_train_epochs):
        ...
        for i, batch in enumerate(dataloader):
            ...
            if num_steps % config.update_freq == 0:
                ...
                #         + (1-config.ema_alpha)*ema_state_dict[key]
