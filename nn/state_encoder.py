...
class GNNStateEncoderV2BGRL(nn.Module):
    """
    Encode state using GNN
    """
    ...
    def __init__(self, node_in_dims: int, edge_in_dims: int, hidden_dims: int) -> None:
        ...
        self.node_prediction_1 = nn.Linear(hidden_dims, hidden_dims)
        self.node_prediction_2 = nn.Linear(hidden_dims, hidden_dims)
    ...
    def forward(self, x: torch.Tensor,
        ...
        # node_proj_embedding = F.relu(self.node_projection_1(output_2))
        # node_proj_embedding = self.node_projection_2(node_proj_embedding)

        node_pred_embedding = F.relu(self.node_prediction_1(output_2))
        node_pred_embedding = self.node_prediction_2(node_pred_embedding)

        return output_2, graph_embedding, node_pred_embedding
